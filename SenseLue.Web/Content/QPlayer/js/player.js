(function($){
	...
	for (var i = 0; i < playlist.length; i++){
        var item = playlist[i];
        $('#playlist').append('<li class="lib" style="overflow:hidden;" id="' + item.id + '"><strong style="margin-left: 5px;">' + item.title + '</strong>&nbsp;-&nbsp;<span style="float: right;" class="artist">' + item.artist + '</span></li>');
		if (item.mp3 == "") {
    ...
    $(".playlist__item_box").click(function () {
        _img = $(this).children("img");
        var _mp3 = { id: _img.attr("id"), title: _img.attr("_title"), artist: _img.attr("_artist"), mp3: _img.attr("_mp3"), cover: _img.attr("src") };

        var bound = true;
        $.each(playlist, function (n, item) {
            if (_mp3.id == item.id) {
                bound = false;
                return false;
            }
        ...
        if (bound == true) {
            playlist.push(_mp3);
            $('#playlist').append('<li class="lib" style="overflow:hidden;" id="' + _mp3.id + '"><strong style="margin-left: 5px;">' + _mp3.title + '</strong>&nbsp;-&nbsp;<span style="float: right;" class="artist">' + _mp3.artist + '</span></li>');
            var index = 0;
